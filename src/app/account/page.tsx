...
const Account = () => {
  ...
  url: process.env.REDIRECT_URL || 'https://nexiler.vercel.app/welcome',
  // This must be true.
  ...
  return (
    <main className="min-h-[115vh] flex items-center justify-center bg-white px-4">
      <div className="w-full max-w-md bg-white border border-gray-200 rounded-2xl shadow-sm p-8">
        ...
        <form onSubmit={(e) => signInWithLink(e)} className="space-y-4">
          ...
          <button
            type="submit"
            className={`w-full cursor-pointer px-4 py-2.5 bg-nexiler text-white rounded-lg font-semibold hover:shadow-[0_0_16px_rgba(135,237,130,0.25)] transition ${loading ? 'opacity-50' : ''}`}
        ...
        >
          {loading ? (
            <div className="flex items-center justify-center">
              <div className="w-5 h-5 border-2 border-nexiler border-t-transparent rounded-full animate-spin" />
            </div>
          ) : (
            <>
